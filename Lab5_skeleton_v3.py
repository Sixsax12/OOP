...
class Account(ABC):
    ...
    def _validate_channel_session(self, channel):
        ...
        """
        if channel.current_card is None:
            raise PermissionError("No card inserted")
        if channel.current_card != self.__card:
            raise PermissionError("Card does not match account")
    ...
    def transfer(self, channel, amount, target_account):
        ...
        if isinstance(channel,(ATM_machine,EDC_machine)):
            if not channel.current_card:
                raise PermissionError("Session Error")
...
class SavingAccount(Account):
    ...
    def _check_withdraw_limit(self, amount):
        ...
        """
        if amount > SavingAccount.WITHDRAW_LIMIT_PER_TRANSACTION:
            raise ValueError("Withdraw limit exceeded (40,000)")
...
class FixedAccount(Account):
    ...
    def _check_withdraw_limit(self, amount):
        ...
        if datetime.now() < self.__maturity_date:
            print("Warning: Early withdrawal from fixed account")
